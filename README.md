# @passmarked/javascript

![NPM](https://img.shields.io/npm/dt/@passmarked/javascript.svg) [![Build Status](https://travis-ci.org/passmarked/javascript.svg)](https://travis-ci.org/passmarked/javascript)
...
* **alert** - The page opened a alert while loading without user interaction, each occurrence is listed with the message presented.
* **log** - Page had console output, which could indicate that debugging was not removed amongst others. Keeping the console clean is the big priority here.
* **error** - Page encountered a Javascript error, returned is the message shown along with the file and line numbers.

The module requires the console,alerts and errors in the payload given. On [Passmarked](https://passmarked.com) this is generated and passed.

The module expects the following properties:
...
```javascript
{
  ...
  console:  [

    {
      ...
      created:  <timestamp when this happened>,
      line:     <line number>,
...
passmarked --filter=javascript example.com
```
...
npm install @passmarked/javascript
```
...
passmarked.createRunner(
  require('@passmarked/javascript'), // this package
...
```bash
git clone git@github.com:passmarked/javascript.git
npm install
npm test
```
...
* [NPM Package](https://www.npmjs.com/package/@passmarked/javascript)
